'''
@Author: WANG Maonan
@Description: IntelliLight reward —— 所有车辆的平均「累计」等待时间（取负）

IntelliLight 相比 PressLight 只把 reward 从 pressure 换成等待时间（与 UniTSA 一致）。
'''
...
def waiting_time_reward(
    ...
    if not tls_dynamic_features_seq:
        raise ValueError("IntelliLight reward history frames must not be empty.")
