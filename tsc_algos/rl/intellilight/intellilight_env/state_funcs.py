...
@Description: IntelliLight movement-based state functions
...
def movement_frame_state(
    ...
    """Build one IntelliLight movement feature frame.
...
def movement_sequence_state(
    ...
    """Build a fixed-length IntelliLight time series state.
    ...
    if not frame_movement_counts:
        raise ValueError("IntelliLight history frames must not be empty.")
    elif len(set(frame_movement_counts)) != 1:
        raise ValueError(
            "Movement count must be the same across IntelliLight history frames, "
    ...
    for offset, movement_features in enumerate(frames):
        ...
        if frame_state.shape != expected_shape:
            raise ValueError(
                f"Unexpected IntelliLight frame shape {frame_state.shape}, "
