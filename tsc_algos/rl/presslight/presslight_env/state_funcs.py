...
@Description: PressLight movement-based state functions
...
def movement_frame_state(
    ...
    """Build one PressLight movement feature frame.
...
def movement_sequence_state(
    ...
    """Build a fixed-length PressLight time series state.
    ...
    if not frame_movement_counts:
        raise ValueError("PressLight history frames must not be empty.")
    elif len(set(frame_movement_counts)) != 1:
        raise ValueError(
            "Movement count must be the same across PressLight history frames, "
    ...
    for offset, movement_features in enumerate(frames):
        ...
        if frame_state.shape != expected_shape:
            raise ValueError(
                f"Unexpected PressLight frame shape {frame_state.shape}, "
