...
@Description: AttendLight reward functions
...
def movement_pressure_penalty(movement_features: List[Dict[str, Any]]) -> float:
    ...
    if controllable_count == 0:
        raise ValueError("AttendLight reward requires at least one controllable movement.")
...
def pressure_reward(
    ...
    if not tls_dynamic_features_seq:
        raise ValueError("AttendLight reward history frames must not be empty.")
