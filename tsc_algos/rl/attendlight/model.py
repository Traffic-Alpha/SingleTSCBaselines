...
@Author: WANG Maonan
@Date: 2026-06-03
@Description: AttendLight movement-token Transformer feature extractor
'''
import torch
...
class AttendLightMovementTransformer(BaseFeaturesExtractor):
    """Encode PressLight movement history with a Transformer.
    ...
    def __init__(
        ...
        if len(observation_space.shape) != 3:
            raise ValueError(
                "AttendLightMovementTransformer expects 3D observations "
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        if history_len != self.history_len or num_movements != self.num_movements:
            raise ValueError(
                "AttendLightMovementTransformer got observation shape "
