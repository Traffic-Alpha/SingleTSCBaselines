...
def movement_sequence_state(
    ...
    if not frame_movement_counts:
        raise ValueError("UniTSA history frames must not be empty.")
    elif len(set(frame_movement_counts)) != 1:
        raise ValueError(
            "Movement count must be the same across UniTSA history frames, "
    ...
    for offset, movement_features in enumerate(frames):
        ...
        if frame_state.shape != expected_shape:
            raise ValueError(
                f"Unexpected UniTSA frame shape {frame_state.shape}, "
